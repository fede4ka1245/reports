...
export function setPayments(sheet, outgoingPayments) {
  ...
  for (let payment of outgoingPayments) {
    ...
    payment.type !== "office" && payment.type !== "members" && createHeaderCell(sheet.getCell(rowIndex, 2), "[id] ФИО");
    payment.type === "members" && createHeaderCell(sheet.getCell(rowIndex, 2), "участник");
    ...
    for (let i = 0; i < payments?.length; i++) {
      payment.type !== "office" && payment.type !== "members" && (
        sheet.getCell(rowIndex, 2).value = `[${payments[i].instructorId}] ${payments[i].name}` || "-"
      );
      payment.type === "members" && (sheet.getCell(rowIndex, 2).value = payments[i].name);
      sheet.getCell(rowIndex, 3).value = `${payments[i].sum} ${payments[i].moneyCode}`;
