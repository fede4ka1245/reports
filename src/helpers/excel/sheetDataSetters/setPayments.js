...
export function setPayments(sheet, outgoingPayments) {
  ...
  for (let payment of outgoingPayments) {
    ...
    payment.type !== "office" && createHeaderCell(sheet.getCell(rowIndex, 2), "[id] ФИО");
    payment.type !== "member" && createHeaderCell(sheet.getCell(rowIndex, 2), "участник");
    ...
    for (let i = 0; i < payments?.length; i++) {
      ...
      payment.type !== "member" && (sheet.getCell(rowIndex, 2).value = payments[i].name);
